...
// Added useState and useEffect for handling client-side state
...
// --- MAIN COMPONENT (Corrected and Responsive) ---
...
const SatellitesTable: FC<SatellitesTableProps> = ({
  ...
  // FIX: Use state and effect to safely calculate height on the client
  const [listHeight, setListHeight] = useState(552); // Default height for SSR

  useEffect(() => {
    // This function calculates and sets the height after the component has mounted
    const updateHeight = () => {
      const tableContainerHeight = Math.min(window.innerHeight * 0.75, 600);
      const headerHeight = 48; // h-12 = 3rem = 48px
      setListHeight(tableContainerHeight - headerHeight);
    };

    // Set the height initially on the client
    updateHeight();

    // Add a listener to update the height if the window is resized
      ...
      <div className="overflow-x-auto">
        <div className="flex w-full min-w-[800px] flex-col rounded-lg bg-[#1A202C] h-[75vh] max-h-[600px]">
          ...
          <div className="flex-grow">
            {data.length > 0 ? (
              <List
                height={listHeight} // Use the client-safe state variable for height
